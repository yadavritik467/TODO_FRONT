...
const MyProfile = () => {
  ...
  const users = async () => {
    try {
      const { data } = await axios.get("https://todo-backend-three-sand.vercel.app/auth/get");
      // console.log(data)
      setUser(data.user);
    } catch (error) {
  ...
  const deleteAccount = async(_id) =>{
    await axios.delete(`https://todo-backend-three-sand.vercel.app/auth/delete/${_id}`)
    await setAuth({
      ...auth,
      user:null,
      token:"",
